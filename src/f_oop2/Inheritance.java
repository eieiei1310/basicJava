...
public class Inheritance {
	...
	public static void main(String[] args) {
		 ...
		 * - 하나의 클래스만 상속받을 수 있다.
		 * - 상속받지 않는 모든 클래스는 Object 클래스를 상속받는다. //Object 클래스는 모든 클래스의 부모 클래스이다.
		 * - 자식 클래스는 부모 클래스의 멤버 외의 새로운 멤버를 가질 수 있으므로 자식 클래스는 부모 클래스보다 크거나 같다.
		 * - 두 개 이상의 클래스를 만드는 데 공통된 멤버가 있는 경우 부모 클래스로 만든다.
		 * 
		 * << super >>
		 ...
		 * 
		 * << 다형성 >>
		 * - 상속으로 인해 여러 가지 형태를 가지게 되는 특징을 말한다.
		 * - 부모 타입으로 생성된 객체는 부모타입에 정의된 멤버만 사용 가능하다.
		 * - 부모 타입과 자식 타입은 서로 형 변환이 가능하다.
		 * - 자식 타입 > 부모타입 : Up - casting (생략가능)
		 * - 부모 타입 > 자식타입 : Down - casting (생략불가)
		...
		am.privateMethod();*/ //-> 프라이빗은 다른 클래스에서 접근 불가
